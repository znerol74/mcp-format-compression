...
def fig1_overview(points):
    ...
    # Quadrant annotations — avoid legend area (upper right)
    ...
    legend_elements = [
        Line2D([0], [0], marker='s', color='w', markerfacecolor='#2ECC71',
               markersize=9, markeredgecolor='white', label='JSON (baseline)'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor=COLORS["toon"],
               markersize=9, markeredgecolor='white', label='TOON (BenchPP)'),
        Line2D([0], [0], marker='^', color='w', markerfacecolor=COLORS["toon"],
               markersize=9, markeredgecolor='white', label='TOON (Universe)'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor=COLORS["tron"],
               markersize=9, markeredgecolor='white', label='TRON (BenchPP)'),
        Line2D([0], [0], marker='^', color='w', markerfacecolor=COLORS["tron"],
               markersize=9, markeredgecolor='white', label='TRON (Universe)'),
    ]
...
def fig4_token_spider(raw):
    """Spider/radar chart of token consumption — one panel per benchmark."""
    fig, axes = plt.subplots(1, 2, figsize=(10, 4.5), subplot_kw=dict(polar=True))
    ...
    benchmarks = [
        ("MCPToolBenchPP", raw["benchpp"], "total_api_tokens"),
        ("MCP-Universe", raw["universe"], "total_api_tokens"),
    ]

    for ax, (bench_name, results, token_key) in zip(axes, benchmarks):
        categories = sorted(set(r["category"] for r in results))
        cat_labels = [c.replace("_", "\n") for c in categories]
        N = len(categories)
        angles = np.linspace(0, 2 * np.pi, N, endpoint=False).tolist()
        ...
        # Normalize to JSON baseline per category (JSON = 1.0)
        json_by_cat = {}
        for cat in categories:
            entries = [r for r in results if r["category"] == cat and r["format"] == "json"]
            if entries:
                json_by_cat[cat] = entries[0].get(token_key, 1)

        for fmt in ["json", "toon", "tron"]:
            values = []
            for cat in categories:
                entries = [r for r in results if r["category"] == cat and r["format"] == fmt]
                if entries and json_by_cat.get(cat, 0) > 0:
                    values.append(entries[0].get(token_key, 0) / json_by_cat[cat])
                else:
                    values.append(0)
            values += values[:1]
        ...
        # Add reference circle at 1.0 (JSON baseline)
        ref_angles = np.linspace(0, 2 * np.pi, 100)
        ax.plot(ref_angles, [1.0] * 100, '--', color='gray', linewidth=0.8, alpha=0.4)

    fig.tight_layout()
    fig.savefig(OUT_DIR / "token_spider.pdf", bbox_inches='tight', dpi=300)
    print(f"Saved {OUT_DIR / 'token_spider.pdf'}")
    plt.close(fig)
